...
module "vpc_production" {
  source                   = "./modules/vpc/"
  vpc_name                 = "vpc-netflix-production"
...
module "iam_production" {
  source                       = "./modules/iam/"
  iam_role_eks_name            = "iam-eks-production"
  iam_role_eks_node_group_name = "iam-eks-node-group-production"
}
...
module "eks_production" {
  source                         = "./modules/eks/"
  eks_name                       = "netflix-production"
  eks_role_arn                   = module.iam_production.role_eks_arn
  ...
  eks_subnets_id                 = module.vpc_production.subnet_private_id
  eks_security_group_id          = [module.vpc_production.security_group_id]
  ...
  eks_node_group_role_arn        = module.iam_production.role_eks_node_group_arn
  ...
  depends_on                     = [module.vpc_production, module.iam_production]
}
